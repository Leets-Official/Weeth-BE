...
public class UserAdminController {
    ...
    public CommonResponse<List<AdminResponse>> findAll() {
        return CommonResponse.createSuccess(ResponseMessage.USER_FIND_ALL_SUCCESS.getStatusCode(),
                ResponseMessage.USER_FIND_ALL_SUCCESS.getMessage(),userUseCase.findAllByAdmin());
    }

    @PatchMapping
    public CommonResponse<Void> accept(@RequestParam Long userId) {
        userUseCase.accept(userId);
        return CommonResponse.createSuccess( ResponseMessage.USER_ACCEPT_SUCCESS.getStatusCode(),
                ResponseMessage.USER_ACCEPT_SUCCESS.getMessage());
    }

    @DeleteMapping
    public CommonResponse<Void> ban(@RequestParam Long userId) {
        userUseCase.ban(userId);
        return CommonResponse.createSuccess(ResponseMessage.USER_BAN_SUCCESS.getStatusCode(),
                ResponseMessage.USER_BAN_SUCCESS.getMessage());
    }

    @PatchMapping("/role")
    public CommonResponse<Void> update(@RequestParam Long userId, @RequestParam String role) {
        userUseCase.update(userId, role);
        return CommonResponse.createSuccess(ResponseMessage.USER_ROLE_UPDATE_SUCCESS.getStatusCode(),
                ResponseMessage.USER_ROLE_UPDATE_SUCCESS.getMessage());
    }
    ...
    public CommonResponse<Void> applyOB(@RequestParam Long userId, @RequestParam Integer cardinal) {
        ...
        return CommonResponse.createSuccess(ResponseMessage.USER_APPLY_OB_SUCCESS.getStatusCode(),
                ResponseMessage.USER_APPLY_OB_SUCCESS.getMessage());
    }

    @PatchMapping("/reset")
    public CommonResponse<Void> resetPassword(@RequestParam Long userId) {
        userUseCase.reset(userId);
        return CommonResponse.createSuccess(ResponseMessage.USER_PASSWORD_RESET_SUCCESS.getStatusCode(),
                ResponseMessage.USER_PASSWORD_RESET_SUCCESS.getMessage());
    }
}
